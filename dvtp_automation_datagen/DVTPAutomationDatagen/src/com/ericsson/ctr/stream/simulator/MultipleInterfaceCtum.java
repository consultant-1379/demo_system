 ...
 * @author ejactho
 */
public class MultipleInterfaceCtum {
    ...
    private List<CtumStream> mmeCtumList = new ArrayList<CtumStream>();
    private int eps = 10;
    ...
    public void runSimulator(String[] args) {
        ...
        try {
            ...
            Logger.getLogger(MultipleInterfaceCtum.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        for (InetAddress addr : addrList) {
            ...
            String mmeId = df.format(Integer.parseInt(octets[2])) + "" + df.format(Integer.parseInt(octets[3]));
            CtumStream s = null;
            try {
                s = new CtumStream(mmeId, addr, host, port);
                ...
                try {
                    ...
                    Logger.getLogger(MultipleInterfaceCtum.class.getName()).log(Level.SEVERE, null, ex1);
                }
                continue;
        ...
        while (true) {
            ...
            for (CtumStream s : mmeCtumList) {
                long lastCount = timeList.get(s.getMmeId());
                long c = s.getEventCount();
                long delta = c - lastCount;
                timeList.put(s.getMmeId(), c);
                System.out.println(time + " - MME " + s.getMmeId() + ", Total : " + df.format(c) + "; Last Minute : " + df.format(delta) + "; events/sec : " + df.format((delta / 60)));
                totalEventCount += c;
            }
    ...
    public static void main(String[] args) {
        new MultipleInterfaceCtum().runSimulator(args);
    }
}
